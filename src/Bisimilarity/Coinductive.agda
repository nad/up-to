...
open import Bisimilarity.Step lts as Step public using (S̲t̲e̲p̲)
...
[]≡↔ {p} {q} {i} (s₁@(_ , lr₁ , rl₁) , f₁) (s₂@(_ , lr₂ , rl₂) , f₂) =
  ...
  (∀ {p′ μ} (p⟶p′ : p [ μ ]⟶ p′) →
   ...
   in ∃ λ (q′₁≡q′₂ : q′₁ ≡ q′₂) →
        subst (q [ μ ]⟶_) q′₁≡q′₂ q⟶q′₁ ≡ q⟶q′₂
          ×
        [ i ] subst (p′ ∼′_) q′₁≡q′₂ (f₁ (inj₁ (_ , p⟶p′ , refl))) ≡′
              f₂ (inj₁ (_ , p⟶p′ , refl)))
    ×
  (∀ {q′ μ} (q⟶q′ : q [ μ ]⟶ q′) →
   let p′₁ , p⟶p′₁ = rl₁ q⟶q′
   ...
   in ∃ λ (p′₁≡p′₂ : p′₁ ≡ p′₂) →
        subst (p [ μ ]⟶_) p′₁≡p′₂ p⟶p′₁ ≡ p⟶p′₂
          ×
        [ i ] subst (_∼′ q′) p′₁≡p′₂ (f₁ (inj₂ (_ , q⟶q′ , refl))) ≡′
              f₂ (inj₂ (_ , q⟶q′ , refl)))                                ↔⟨⟩
  ...
  lemma₅₁ {p′} {μ} {p⟶p′} q′₁≡q′₂ =
    ...
    ([ i ] f₁ (inj₁ (μ , p⟶p′ , refl)) ≡′
           subst (p′ ∼′_) (sym q′₁≡q′₂) (f₂ (inj₁ (_ , p⟶p′ , refl))))  ≡⟨ lemma″ q′₁≡q′₂ ⟩∎
    ...
    lemma′ :
      ...
      f₂ (inj₁ (μ , p⟶p′ , sym q′₁≡q′₂))
        ≡
      subst (p′ ∼′_) (sym q′₁≡q′₂) (f₂ (inj₁ (_ , p⟶p′ , refl)))
    lemma′ refl = refl

    lemma″ :
      ∀ {q′₂ x} (q′₁≡q′₂ : proj₁ (lr₁ p⟶p′) ≡ q′₂) →
      ([ i ] f₁ (inj₁ (μ , p⟶p′ , refl)) ≡′ subst (p′ ∼′_) (sym q′₁≡q′₂) x)
        ≡
      ([ i ] subst (p′ ∼′_) q′₁≡q′₂ (f₁ (inj₁ (_ , p⟶p′ , refl))) ≡′ x)
    lemma″ refl = refl
  ...
  lemma₅₂ {q′} {μ} {q⟶q′} p′₁≡p′₂ =
    ...
    ([ i ] f₁ (inj₂ (μ , q⟶q′ , refl)) ≡′
           subst (_∼′ q′) (sym p′₁≡p′₂) (f₂ (inj₂ (_ , q⟶q′ , refl))))  ≡⟨ lemma″ p′₁≡p′₂ ⟩∎
    ...
    lemma′ :
      ...
      f₂ (inj₂ (μ , q⟶q′ , sym p′₁≡p′₂))
        ≡
      subst (_∼′ q′) (sym p′₁≡p′₂) (f₂ (inj₂ (_ , q⟶q′ , refl)))
    lemma′ refl = refl

    lemma″ :
      ∀ {q′₂ x} (p′₁≡p′₂ : proj₁ (rl₁ q⟶q′) ≡ q′₂) →
      ([ i ] f₁ (inj₂ (μ , q⟶q′ , refl)) ≡′ subst (_∼′ q′) (sym p′₁≡p′₂) x)
        ≡
      ([ i ] subst (_∼′ q′) p′₁≡p′₂ (f₁ (inj₂ (_ , q⟶q′ , refl))) ≡′ x)
    lemma″ refl = refl

module Bisimilarity-of-∼ {p q} {i : Size} (p∼q₁ p∼q₂ : p ∼ q) where
